 ...
 * RobotBarista 클래스는 Barista 인터페이스의 구현체로, 음료 제조라는 핵심 책임을 가집니다.
...
public class RobotBarista implements Barista {
	// 음료 제조 외에 부가적인 역할(기능)이 너무 많다. ( 재시도, 완료 로그 )
	@Override
	public Drink prepare(OrderSlip slip) {
		...
		while (true) {
			try {
				Util.randomException("RobotBarista::prepare"); // 50% 확률로 예외 발생 시뮬레이션
				break; // 성공 시 루프 탈출
			} catch (Exception e) {
				retryCount++;
		...
		Util.LoggingCompleted("RobotBarista::prepare");
